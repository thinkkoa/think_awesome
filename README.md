...
[think_payload](https://github.com/thinkkoa/think_payload)  | ✔️（embedding） | Payload parser for ThinkKoa.ThinkKoa querystring以及body解析中间件。能够解析querystring、form、file、json等多种常用数据。
...
[think_static](https://github.com/thinkkoa/think_static)  | ✔️（embedding）|  Static resource service for ThinkKoa.ThinkKoa静态文件服务中间件。生产环境建议使用nginx进行处理。
[think_trace](https://github.com/thinkkoa/think_trace)   | ✔️（embedding） |   Trace for ThinkKoa.ThinkKoa错误处理及拦截中间件。
...
[think_cache](https://github.com/thinkkoa/think_cache)   | ✔️  |  缓存中间件，缓存链接初始化封装，方便应用中使用
